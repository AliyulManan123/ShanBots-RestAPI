...
function checkAuth(req, res, next) {
    if (req.isAuthenticated()) {
        ...
        req.flash('error_messages', "Silahkan login untuk melanjutkan !");
        res.redirect('/login');
    }
}
...
router.post('/signup', recaptcha.middleware.verify, captchaRegister, async(req, res) => {
    ...
    if (!email || !username || !password || !confirmpassword) {
        ...
        req.flash('error_messages',"Passo Tidak samac !");
        res.redirect('/signup');
    } else if(!checkpw ) {
        ...
        req.flash('error_messages',"Nama pengguna Tidak dibenarkan menggunakan emoji");
        res.redirect('/signup');  
    }else if(!checkemail){
        ...
        user.findOne({ $or: [{ email: email }, { username: username }] }, function (err, data) {
            if (err) throw err;
            if (data) {
                req.flash('error_messages',"Pengguna Sudah Ada, Coba Masuk !");
                res.redirect('/signup');
            } else {
...
router.get('/send-verification-email', checkAuth, async (req, res) => {
    ...
    if (req.user.isVerified ) {
        ...
        if (check) {
        req.flash('error_messages', 'Tolong Jangan Spam Tunggu Setelah 30 menit.')
        res.redirect('/docs');
        }else{
        ...
        if(mail == 'error'){
            req.flash('error_messages','Error Silakan coba lagi besok');
            res.redirect('/docs');
        }else{
        req.flash('success_messages', 'Done, Tautan Email Terkirim, Link Kedaluwarsa Setelah 30 menit.')
        res.redirect('/docs');
        }
...
if (userData) {
if (Cooldown) {
    req.flash('error_messages','Mohon Jangan Spam Tunggu Setelah 30 menit.');
    res.redirect('/forgot-password')
